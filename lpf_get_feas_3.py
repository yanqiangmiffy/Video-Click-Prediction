...
def get_fea(train, test, user, app):
    ...
    for i in tqdm(cat_list):
        ...
        train['{}_count'.format(i)] = train.groupby(['{}'.format(i)])['id'].transform('count')
        test['{}_count'.format(i)] = test.groupby(['{}'.format(i)])['id'].transform('count')

    for i in ['newsid']:
        train['{}_count'.format(i)] = train.groupby(['{}'.format(i)])['id'].transform('count')
        test['{}_count'.format(i)] = df[df['ts_day'].isin([9, 10, 11])].groupby(['{}'.format(i)])['id'].transform('count')

    train['newsid'] = train.apply(lambda x: np.nan if x.newsid_count == 1 else x.newsid, axis=1)
    test['newsid'] = test.apply(lambda x: np.nan if x.newsid_count == 1 else x.newsid, axis=1)
